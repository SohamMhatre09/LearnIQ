...
if [ ! -f "/home/ubuntu/learniq-backend/package.json" ]; then
    echo "❌ Not in the correct deployment directory"
    exit 1
fi

echo "1. Stopping Nginx to clear any issues..."
sudo systemctl stop nginx

echo "2. Removing any existing SSL certificates for clean setup..."
sudo certbot delete --cert-name $DOMAIN 2>/dev/null || echo "No existing certificate to delete"

echo "3. Ensuring webroot directory exists..."
sudo mkdir -p /var/www/html/.well-known/acme-challenge
sudo chown -R www-data:www-data /var/www/html

echo "4. Creating fresh Nginx configuration..."
...
if [ $? -eq 0 ]; then
    ...
    echo "12. Checking what's using port 80..."
